...
(수정 이력, 2026-09-20) 준비 완료 상태에서 보드/팔레트가 통째로 비활성화돼 있어서 재배치로 isReady를 되돌리는 경로가 실제로는 눌리지 않던 버그를 003-turn-gameplay 작업 중 발견해 수정했다(보드 상태를 RoomPage로 끌어올리면서 disabled 조건도 제거).
