...
flowchart LR
    ...
    B["클라이언트 B"]
    C["클라이언트 C"]
    RT(["Supabase Realtime<br/>bingo-room-{roomId}<br/>Broadcast + Presence<br/>(중계만 함, 상태 저장 없음)"])

    A <--> RT
    B <--> RT
    C <--> RT

    HostNote["호스트만 하는 일<br/>· 턴 타이머 관리 · 시간초과 자동호출<br/>· 동시우승 판정 · game-over 확정"]
    A -.- HostNote
```
...
stateDiagram-v2
    [*] --> waiting
    waiting --> playing: 전원 준비 완료 (game-started)
    playing --> ended: 3줄 완성 또는 활성 인원 1명 (game-over)
    ended --> waiting: 다시 하기 (restart-game)
```
...
```mermaid
sequenceDiagram
    participant P as 턴 유저
    participant CH as Realtime 채널
    participant H as 호스트
    participant O as 다른 참가자들

    Note over P,O: 턴 시작 (turnSeq=N, turnStartedAt=T)

    alt 10초 안에 스스로 호출
        P->>CH: number-called (auto=false, turnSeq=N)
        CH-->>H: 수신
        CH-->>O: 수신
    else 10초 경과, 아직 미호출
        Note over H: T+10s 시점에 turnSeq=N이<br/>아직 처리 안 됐음을 확인
        H->>CH: number-called (auto=true, turnSeq=N)
        CH-->>P: 수신
        CH-->>O: 수신
    end

    Note over P,O: 전원 로컬에서 마킹 + 라인 수 재계산<br/>turnSeq=N+1로 다음 턴 시작
```
...
```mermaid
flowchart TD
    Start(["방 링크 접속 + 닉네임 입력"]) --> CheckDup{"같은 닉네임이<br/>이미 접속 중?"}
    CheckDup -- 예 --> RejectDup["입장 거부<br/>이미 사용 중인 닉네임"]
    CheckDup -- 아니오 --> CheckFull{"정원(5명) 초과?"}
    CheckFull -- 예 --> RejectFull["입장 거부<br/>방이 가득 찼습니다"]
    CheckFull -- 아니오 --> CheckStarted{"이미 시작/종료된 방?"}
    CheckStarted -- 아니오 --> JoinWaiting["대기실 참가자로 입장<br/>(roomStatus: waiting)"]
    CheckStarted -- 예 --> RequestState["request-state 전송<br/>최대 3초 대기"]
    RequestState --> GotSync{"state-sync<br/>응답 받음?"}
    GotSync -- 타임아웃 --> FallbackWaiting["대기 상태로 입장<br/>+ 안내 메시지"]
    GotSync -- 예 --> CheckMember{"turnOrder에<br/>내 닉네임 있음?"}
    CheckMember -- 아니오 --> RejectStranger["입장 거부<br/>이미 시작된 방"]
    CheckMember -- 예 --> Rejoin["원래 참가자로 인정<br/>공유 상태 반영 후 이어서 참여"]
```
